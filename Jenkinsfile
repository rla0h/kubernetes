        ...
        stage('Apply Source Pub') {
            steps {
                script {
                    ...
                    sh "kubectl exec -it ${pubpodName} -- sh -c 'cd ../NWT/bin && java -ea -cp classes:/DDS/NWT/lib/*:/DDS/NWT/bin:class -Djava.library.path=/DDS/OpenDDS-DDS-3.23.1/lib/libOpenDDS_Dcps.so.3.23.1 NWT_TestPublisher -DCPSConfigFile tcp.ini -DCPSTransportDebugLevel 0 -w'"                    
                }
            }
        }
        ...
        stage('Apply Source Sub') {
            steps {
                script {
                    ...
                    sh "kubectl exec -it ${subpodName} -- sh -c 'cd ../NWT/bin && java -ea -cp classes:/DDS/NWT/lib/*:/DDS/NWT/bin:classes -Djava.library.path=/DDS/OpenDDS-DDS-3.23.1/lib/libOpenDDS_Dcps.so.3.23.1 NWT_TestSubscriber -DCPSConfigFile tcp.ini -DCPSTransportDebugLevel 0 -r'"
                }
            }
        }
