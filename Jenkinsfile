        ...
        stage('Apply Source Pub') {
            steps {
                script {
                    ...
                    sh "kubectl exec -it ${pubpodName} -- sh -c 'java -ea -cp classes:/DDS/NWT/lib/*:/DDS/NWT/bin:classes -Djava.library.path=$DDS_ROOT/lib /DDS/NWT/bin/NWT_TestPublisher.class -DCPSConfigFile /DDS/NWT/bin/tcp.ini -DCPSTransportDebugLevel 0 -w'"                    
                }
            }
        }
        ...
        stage('Apply Source Sub') {
            steps {
                script {
                    ...
                    sh "kubectl exec -it ${subpodName} -- sh -c 'java -ea -cp classes:/DDS/NWT/lib/*:/DDS/NWT/bin:classes -Djava.library.path=$DDS_ROOT/lib /DDS/NWT/bin/NWT_TestSubscriber.class -DCPSConfigFile /DDS/NWT/bin/tcp.ini -DCPSTransportDebugLevel 0 -r'"
                }
            }
        }
