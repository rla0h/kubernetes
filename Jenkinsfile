        ...
        stage('Apply Source Pub') {
            steps {
                script {
                    ...
                    sh "kubectl exec -it ${pubpodName} -- sh -c 'java -ea -cp classes:/DDS/NWT/lib/*:/DDS/NWT/bin:classes -Djava.library.path=$DDS_ROOT/lib ../NWT/bin/NWT_TestPublisher -DCPSConfigFile ../NWT/bin/tcp.ini -DCPSTransportDebugLevel 0 -w'"                    
                }
            }
        }
        ...
        stage('Apply Source Sub') {
            steps {
                script {
                    ...
                    sh "kubectl exec -it ${subpodName} -- sh -c 'java -ea -cp classes:/DDS/NWT/lib/*:/DDS/NWT/bin:classes -Djava.library.path=$DDS_ROOT/lib ../NWT/bin/NWT_TestSubscriber -DCPSConfigFile ../NWT/bin/tcp.ini -DCPSTransportDebugLevel 0 -r'"
                }
            }
        }
